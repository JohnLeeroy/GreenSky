...
import org.ups.greensky.weather.getIcon
    ...
    override fun bind(model: OverviewItem) {
        ...
        when (model) {
            is DailyWeatherItem -> {
                ...
                itemView.dailyWeatherIcon.setImageResource(model.precipType.getIcon())
            }
            is CurrentWeatherItem -> {
                itemView.tempLabel.text = model.temperature
                itemView.weatherLabel.text = model.summary
                itemView.weatherIcon.setImageResource(model.precipType.getIcon())
            }
        }
    }
